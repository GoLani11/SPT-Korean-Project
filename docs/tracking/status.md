...
- Release smoke testing should visually confirm the new `한국어 (한영 병기)` option, immediate two-way switching, persisted selection, and adjusted UI inside a launcher-started game; the automated client check proves the 4.1.3 font target exists but does not render game screens.
