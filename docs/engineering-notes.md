...
The server package references are aligned to `4.0.13`. The server mod metadata keeps `SptVersion` at `~4.0.0`, so it accepts the 4.0.x compatibility family while avoiding a false claim for later SPT release families.
