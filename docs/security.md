...
Release packages write only the version's Korean server-mod folder, its two locale payloads, and `BepInEx\plugins\GoLani.KoreanModFix.dll` when the user extracts them. They contain no installer, command script, executable, absolute path, parent traversal, SPT database replacement, BepInEx core file, or unrelated mod.
