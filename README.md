...
SPT 서버 한글 번역과 한국어 UI 표시 보정을 하나의 패키지로 제공해요. Golani와 Makina가 제작하고 Gomeng이 배포에 도움을 주셨어요.

번역은 계속 보완하고 있어요. 오역이나 미번역을 발견하면 사용 중인 SPT 버전, 표시된 문구와 위치를 [Issues](../../issues)에 남겨 주세요.

### 2.0.1 주요 변경 사항

- 지원하는 모든 SPT 버전의 실제 퀘스트 조건을 의미 단위로 다시 대조하고, 이름은 같지만 진영이나 요구 조건이 다른 퀘스트를 구분했어요.
- 특정 아이템이나 장비를 요구하는 퀘스트 목표는 현재 한글 아이템 `Name`과 같은 명칭을 사용하도록 맞췄어요.
- SPT 4.1.0 전용 패키지와 SPT 4.1.2–4.1.3 공용 패키지를 분리하고 각각 별도의 호환성 검사를 적용했어요.
- 배포 파일명을 `SPT-KR-<버전>.zip`과 `SPT-KR-EN-<버전>.zip` 형식으로 줄였어요.
- 총 14개 ZIP의 번역 키 순서, 내부 경로, 대상 DLL과 SHA-256을 자동 검증하도록 배포 과정을 강화했어요.

### 다운로드 파일 선택

SPT 3.x, 4.0.13, 4.1.0은 설치 버전과 **정확히 같은 ZIP 하나**를 받으세요. SPT 4.1.2와 4.1.3은 두 버전이 함께 표시된 공용 ZIP을 사용하세요. SPT 4.1.1은 지원하지 않아요.
...
- `KR`: 기본 한글판이에요. 기존 형식에 따라 퀘스트 제목·목표, 아이템 설명의 영문 머리말과 일부 레이드 탈출구에는 영어가 함께 표시될 수 있어요.
- `KR-EN`: 전체 한영 병기판이에요.

두 판을 동시에 설치하지 마세요. 사용 중인 SPT와 다른 버전의 ZIP이나 다른 한국어 로케일 모드도 함께 설치하면 안 돼요.
...
ZIP에는 선택한 SPT 버전에 필요한 파일만 들어 있으며 설치용 BAT나 EXE는 포함하지 않아요. 기존 `spt_korean_localization_G&M` 또는 `SPT_Korean_Localization` 정식 버전은 같은 경로에 덮어쓸 수 있어요.
...
SPT 4.1.0 ZIP에는 4.1.0 전용 서버 모드와 클라이언트 플러그인이 들어 있어요. 4.1.2–4.1.3 ZIP은 두 버전에서 공용으로 검증한 바이너리와 번역을 사용해요. 서로 바꿔 설치하거나 표시되지 않은 SPT 버전에 설치하지 마세요.

### 개발용 릴리스 빌드

빌드에는 .NET 10 SDK, SPT 클라이언트 참조 파일과 같은 상위 폴더의 `spt-korean-translate` 저장소가 필요해요. `make-release-packages.bat`은 `D:\SPT3.8.3`을 먼저 사용하고, 없으면 `D:\SPT`를 사용해요.

```powershell
..\spt-korean-translate\.venv\Scripts\python.exe .\tools\package_release_versions.py
```

또는 `make-release-packages.bat`을 실행하세요. 생성된 ZIP 14개와 검증 요약은 `artifacts\release`에 저장되며 Git에는 포함되지 않아요.
