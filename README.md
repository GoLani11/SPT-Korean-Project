...
**2.1.1 배포 준비 완료 — 공개 업로드 대기**

[Releases](../../releases) · [Issues](../../issues) · [설치 안내](docs/releases/2.1.1-install.md)

Golani가 제작하고 Gomeng이 배포를 돕는 SPT 한글화 프로젝트입니다. **`SPT-KR-2.1.1.zip` 하나**에 기본 한글판, 전체 한영 병기판과 한국어 UI 보정을 제공합니다.

## 설치와 업데이트

1. 게임과 서버를 종료합니다.
2. 기존 서버 한글화 모드, 테스트용 서버 안내 모듈과 중복 클라이언트 DLL을 모드 검색 경로 밖에 백업한 후 제거합니다. 정확한 경로는 [설치 안내](docs/releases/2.1.1-install.md)를 따르세요.
...
두 방식은 게임 재시작이나 재설치 없이 전환할 수 있습니다. ZIP은 `BepInEx/plugins` 아래의 DLL·번역·라이선스만 포함합니다. 설치 방법은 릴리즈 안내에서 확인할 수 있습니다. 설치 프로그램과 서버 모듈은 없습니다.

서버 콘솔 안내 대신 게임의 `BepInEx/LogOutput.log`에 모드 버전과 번역 초기화 결과를 기록합니다.

## 다른 모드와 함께 사용하기

2.1.1부터 다른 모드가 수정하거나 추가한 문구는 자동으로 보존하고, 기본 문구에는 한글화를 적용합니다. 별도 설정이나 열쇠 번역 삭제는 필요하지 않습니다. 한국어·한영 병기 전환과 이후의 부분 업데이트에도 같은 규칙을 적용합니다.

다른 모드의 문구는 제공된 언어 그대로 표시됩니다. 게임 화면을 직접 바꾸는 클라이언트 모드나 기본 비교 파일 자체를 수정하는 방식까지 모두 감지하는 것은 아닙니다.
...
상세 변경 사항은 [2.1.1 릴리즈 본문](docs/releases/2.1.1-release-notes.md), 구현과 검증 범위는 [클라이언트 한글화 문서](docs/client-locale-prototype.md)를 참고하세요.
